\chapter{Density Estimation in Kernel Exponential Families}
\label{ch2}
...
Let $\calX \subseteq \Real^d$ be the sample space and $\varphi: \calX \to \Real^m$ be a measurable vector-valued function such that $\varphi \parens{x} = \parens{\varphi_1 \parens{x}, \cdots, \varphi_m \parens{x}}^\top \in \Real^m$ for all $x \in \calX$. An \textit{$m$-dimensional exponential family} \parencite{Brown1986-ef, Barndorff-Nielsen2014-ua}, denoted by $\mathcal{Q}_{\mathrm{fin}}$, in its natural parametrization form, contains all pdfs of the form 
...
\subsection{Properties of Kernel Exponential Families}
...
\subsubsection{Characterization of the Natural Parameter Space for Bounded Kernels}

The first property we discuss here is related to the characterization of $\calF$ when a bounded kernel function $k$ is used, where $k$ is said to be \emph{bounded} if $\kappa_1 := \sup_{x \in \calX} \sqrt{k \parens{x, x}} < \infty$. 
...
\begin{align*}
	f \parens{x} = \sum_{i=1}^{\infty} y_i x, \qquad \text{ for all } x \in \calX, 
\end{align*}
...
\subsubsection{Convexity of the Log-partition Functional}
...
As we will see in Subsection \ref{subsection-nnl-kef} of this chapter and Chapter \ref{ch4}, the convexity of $A$ plays an important role in density estimation using the (penalized) NLL loss functional, which ensures the convexity of the (penalized) NLL loss functional and directly relates to the existence and the uniqueness of its minimizer. 

\subsubsection{Differential Properties of the Log-partition Functional}
...
To start with, notice that the domain of $A$ is a collection of functions over $\calX$, or more precisely, a subset of a Hilbert space. We need a version of differentiability defined for functionals whose domain is a normed space or a subset of it. We choose the \textit{Frech{\'e}t differentiability}, whose definition, together with those of Fr{\'e}chet derivative and gradient, is given in Section \ref{section-frechet-differentiable} in Appendix \ref{app1}. 

The following lemma illustrates these definitions and serves as a preparation for the derivation of the Fr{\'e}chet derivative and gradient of $A$ in Proposition \ref{prop-frechet-derivative-A}. 

\begin{lemma}\label{lemma-eval-functional-frechet-diff}
	Suppose $\sup_{x \in \calX} \sqrt{k \parens{x, x}} < \infty$ so that $\calF = \calH$. Let $x \in \calX$ be fixed and $J_x: \calH \to \Real$ be the evaluation functional $J_x \parens{f} = \innerp{f}{k \parens{x, \,\cdot\,}}_{\calH} = f \parens{x}$ for all $f \in \calH$. Then, $J_x$ is Frech{\'e}t differentiable over $\calH$ with the Frech{\'e}t derivative at $f \in \calH$ being 
	\begin{align*}
		\Diff J_x \parens{f} \parens{g} = \innerp{g}{k \parens{x, \,\cdot\,}}_{\calH}, \quad \qquad \text{ for all } g \in \calH. 
	\end{align*}
	In addition, the Fr{\'e}chet gradient is $\nabla J_x: \calH \to \calH$ with $\nabla J_x \parens{f} = k \parens{x, \,\cdot\,}$ for all $f \in \calH$. 
\end{lemma}

The proof of Lemma \ref{lemma-eval-functional-frechet-diff} can be found in Section \ref{subsection-proof-lemma-eval-functional-frechet-diff}. 
...
The proof of Proposition \ref{prop-frechet-derivative-A} can be found in Section \ref{subsection-proof-prop-frechet-derivative-A}. 

\begin{remark}
	Note that the integrand of $\int_{\calX} k \parens{x, \,\cdot\,} q_f \parens{x} \diff x$ is an element in $\calH$ and this integral is a Bochner integral \parencite{Diestel1977-bt, Denkowski2013-ke}; see Section \ref{section-bochner-integral} in Appendix \ref{app1} for its definition and properties. In particular, the second equality in \eqref{eq-frechet-derivative-A} follows from Proposition \ref{prop-properties-bochcher-integral}\ref{prop-properties-bochcher-integral-c} there. 
...
\begin{proposition}[Second-order Fr{\'e}chet differentiability, derivative and gradient of $A$]\label{prop-frechet-derivative-A-order2} 
...
The proof of Proposition \ref{prop-frechet-derivative-A-order2} is similar to that of Proposition \ref{prop-frechet-derivative-A} and is omitted here. 

The bounded linear operator $\nabla^2 A \parens{f}$, or more generally, the operator 
\begin{align*}
	\Sigma_F := \parens[\bigg]{\int_{\calX} k \parens{x, \,\cdot\,} \otimes k \parens{x, \,\cdot\,} \diff F \parens{x} } - \parens[\bigg]{\int_{\calX} k \parens{x, \,\cdot\,} \diff F \parens{x} } \otimes \parens[\bigg]{\int_{\calX} k \parens{x, \,\cdot\,} \diff F \parens{x} }
\end{align*}
where $F$ is a distribution over $\calX$, maps from $\calH$ to itself, and is referred to as the \textit{covariance operator} in the literature, since, for any $f, g \in \calH$, we have 
\begin{align*}
	\innerp{f}{\Sigma_F g}_{\calH} = \E_F \bracks{f \parens{X} g \parens{X}} - \E_F \bracks{f \parens{X}} \E_F \bracks{g \parens{X}}, 
\end{align*}
which is the covariance between $f \parens{X}$ and $g \parens{X}$ with $X \sim F$. The operator $\Sigma_F$ is known to be linear, bounded, self-adjoint, and of trace-class \parencites{Baker1971-pu}. The covariance operator has been used in such statistical and machine learning applications as dimensionality reduction \parencites{Fukumizu2004-zw, Fukumizu2009-zh}, kernel principal component analysis \parencites{Scholkopf1998-jn}, kernel canonical correlation analysis \parencites{Fukumizu2007-rd}, and independence and conditional independence measures \parencites{Gretton2005-tj, Fukumizu2007-rc}. We will also see the covariance operator appears in the influence function of the ML density projection in $\calQ_{\ker}$ in Chapter \ref{ch5}. More details on the covariance operator and, more generally, the cross-covariance operator can be found in Section 3.2 and 4.3 in \textcites{kernel-mean-embedding-survey-muandet}. 
...
\begin{align*}
	\tilde{q}_{\theta} \parens{x} = & \, \mu \parens{x} \exp \parens[\big]{\innerp{\theta}{\varphi \parens{x}} - B \parens{\theta}}  \\ 
	= & \, \mu \parens{x} \exp \parens[\bigg]{\sum_{j=1}^m \theta_j \varphi_j \parens{x} - B \parens{\theta}} \\ 
	= & \, \mu \parens{x} \exp \parens[\big]{\innerp{f}{k \parens{x, \,\cdot\,}}_{\calH_0} - A \parens{f}}, 
\end{align*}
where $\theta := \parens{\theta_1, \cdots, \theta_m}^\top \in \Theta \subseteq \Real^m$, $f := \sum_{j=1}^m \theta_j \varphi_j \in \calH_0$, $k \parens{x, \,\cdot\,} := \sum_{j=1}^m \varphi_j \parens{x} \varphi_j \in \calH_0$, and 
\begin{align*}
	A \parens{f} = & \, \log \parens[\bigg]{\int_{\calX} \mu \parens{x} \exp \parens[\big]{\innerp{f}{k \parens{x, \,\cdot\,}}_{\calH_0} } \diff x} \\ 
	= & \, \log \parens[\bigg]{\int_{\calX} \mu \parens{x} \exp \parens[\bigg]{ \sum_{j=1}^m \theta_j \varphi_j \parens{x} } \diff x} = B \parens{\theta}. 
\end{align*}
Thus, every $\tilde{q}_{\theta} \in \calQ_{\mathrm{fin}}$ can be written in the form of pdfs in $\calQ_{\ker}$, and, with the RKHS being $\calH_0$, $\calQ_{\mathrm{fin}}$ is a special case of $\calQ_{\ker}$. 

In the rest of this section, we provide several examples of frequently used finite-dimensional exponential families to illustrate Proposition \ref{prop-fin-ker-exp-fam-connection}. In these examples, we temporarily ignore the fact that $\mu$ is a pdf over $\calX$, which is assumed in the definitions of $\calQ_{\mathrm{fin}}$ and $\calQ_{\ker}$. 

\begin{example}[Binomial distribution]
	Consider the family of pmfs of the binomial distribution with the known number of trials $n$ and the success probability $\eta \in \parens{0, 1}$. The general form of the pmf is 
	\begin{align*}
	...
	Furthermore, with $f = \theta \cdot \mathrm{Id}$ for some $\theta \in \Real$, we have 
	\begin{align*}
...
\begin{example}[Exponential distribution]
	...
	\begin{align*}
		p_{\eta} \parens{x} = \frac{1}{\eta} \exp \parens[\bigg]{-\frac{x}{\eta}}, \qquad \text{ for all } x \in \calX := \sets[\big]{x \mid x > 0}. 
	\end{align*}
	...
	Finally, with $f = \theta \cdot \mathrm{Id}$ for some $\theta \in \Theta$, we have 
	\begin{align*}
		A \parens{f} = \log \parens[\bigg]{\int_{\calX} \mu \parens{x} \exp \parens{f \parens{x}} \diff x} 
...
\begin{example}[Univariate normal distribution]
	...
	where 
	\begin{align*}
		\theta := & \, \parens{\theta_1, \theta_2}^\top = \parens[\bigg]{-\frac{1}{2\sigma^2}, \frac{\eta}{\sigma^2}}^\top, \qquad \quad \Theta := \sets[\bigg]{\theta \biggm| \theta := \parens{\theta_1, \theta_2}^\top, \theta_1 < 0, \theta_2 \in \Real}. \\ 
	...
	\end{align*}
	for some $\parens{\theta_1, \theta_2}^\top \in \Theta$. For any $f, g \in \calH_0$ with $f \parens{x} = \theta_{1,1} x^2 + \theta_{1, 2} x$ and $g \parens{x} = \theta_{2, 1} x^2 + \theta_{2, 2} x$ for all $x \in \calX$, where $\parens{\theta_{1,1}, \theta_{1,2}}^\top \in \Theta$ and $\parens{\theta_{2,1}, \theta_{2,2}}^\top \in \Theta$, define the inner product between $f$ and $g$ to be 
	\begin{align*}
...
In the rest of this dissertation, we are going to make the following assumptions on $\calH$, $k$ and $\mu$, unless explicitly stated otherwise: 
\begin{enumerate}[label=\textbf{(A\arabic*)}]
	\item \label{assumption-2-1-inf-dim} The RKHS $\calH$ is infinite-dimensional. 
	\item \label{assumption-2-2-bdd-kernel} The kernel function $k$ is continuous and bounded, i.e., $\kappa_1 := \sup_{x \in \calX} \sqrt{k \parens{x, x}} < \infty$. 
	\item \label{assumption-2-3-no-const-fun} The RKHS $\calH$ does \emph{not} contain nonzero constant functions, i.e., $1 \notin \calH$. 
	\item \label{assumption-2-4-base-density} The base density $\mu$ is a continuously differentiable pdf over $\calX$ with the support being $\calX$. 
\end{enumerate}

If we assume $\calH$ to be finite-dimensional, rather than infinite-dimensional as we have done in \ref{assumption-2-1-inf-dim}, we are returning to the case of finite-dimensional exponential families discussed in Section \ref{subsection-fin-dim-exp-fam}, as we have seen in Section \ref{subsection-connection-fin-exp-fam}. Classic theories on the estimation in $\calQ_{\mathrm{fin}}$ are directly applicable and are not very interesting. Thus, we rule out this case. 

The main motivation of \ref{assumption-2-2-bdd-kernel} is to ensure $\calF = \calH$, as we have shown in Proposition \ref{prop-bounded-kernel-H=F}. This is going to make all the optimization problems considered in Section \ref{section-density-estimation} unconstrained. In addition, since $\calX \subseteq \Real^d$ and $k$ is continuous, Lemma 4.33 in \textcite{Steinwart2008-tn} implies that $\calH$ is separable and has an orthonormal basis \parencite[Theorem 11 in][]{Royden2018-cd}. 

%Assumptions \ref{assumption-2-3-no-const-fun} and \ref{assumption-2-4-base-density} together ensure the identifiability of $\calQ_{\ker}$, i.e., $q_{f_1} = q_{f_2}$ if and only if $f_1 = f_2$. To see this, first note it is trivial that $f_1 = f_2$ implies $q_{f_1} = q_{f_2}$. Conversely, suppose $q_{f_1} = q_{f_2}$. Then, we have 
%\begin{align*}
%	1 = \frac{q_{f_1} \parens{x}}{q_{f_2} \parens{x}} = \exp \parens[\Big]{ \parens[\big]{ f_1 \parens{x} - f_2 \parens{x} } - \parens[\big]{A \parens{f_1} - A \parens{f_2}} } \qquad \text{ for all } x \in \calX, 
%\end{align*}
%which is equivalent to 
%\begin{align*}
%	f_1 \parens{x} - f_2 \parens{x} = \innerp{f_1 - f_2}{k \parens{x, \,\cdot\,}}_{\calH} = 0 \qquad \text{ for all } x \in \calX, 
%\end{align*}
%implying $f_1 = f_2$. One sufficient condition that guarantees \ref{assumption-2-3-no-const-fun} is that the kernel function $k$ is continuous on $\calX \times \calX$ and vanishes at infinity \parencite[see Remark 3(iii) in][]{Sriperumbudur-density-estimation-inf-exp-family}. 



\section{Nonparametric Density Estimation in Kernel Exponential Families}\label{section-density-estimation}


% So far, we have only focused on the properties of $\calQ_{\ker}$ and its connection with $\calQ_{\mathrm{fin}}$. 
We now turn to the nonparametric density estimation problem in $\calQ_{\ker}$. Borrowing the framework in Chapter \ref{ch1}, we consider the following minimization problem 
...
The very first question we consider is how rich $\calQ_{\ker}$ is as a class of pdfs to estimate $p_0$. More specifically, we want to understand what class of density functions over $\calX$ can be approximated arbitrarily well by those in $\calQ_{\ker}$. Proposition 1, Corollary 2 and Proposition 13 in \textcite{Sriperumbudur-density-estimation-inf-exp-family} answered this question and state that, under certain regularity conditions, 
...
\subsection{Density Estimation Using the Negative Log-likelihood Loss Functional}\label{subsection-nnl-kef}
...
Since $A$ is convex by Proposition \ref{prop-convexity-A}, and $- \frac{1}{n} \sum_{i=1}^n f \parens{X_i} = - \frac{1}{n} \sum_{i=1}^n \innerp{f}{k \parens{X_i, \,\cdot\,}}_{\calH}$ is linear and, hence, convex in $f$, their sum, $\widehat{J}_{\NLL}$, is convex as well. 

We first have a bad news stated in the following proposition. 

\begin{proposition}[\textcite{Fukumizu2005-mf}]\label{prop-no-mle-inf-exp-fam}
	Under \ref{assumption-2-1-inf-dim} and \ref{assumption-2-2-bdd-kernel}, the infimum of $\widehat{J}_{\NLL}$ is \emph{not} attained in $\calH$. 
\end{proposition}

The direct consequence of Proposition \ref{prop-no-mle-inf-exp-fam} is that the ML density estimator in $\calQ_{\ker}$ does \emph{not} exist. 
...
\begin{remark}
	...
	\end{align}
	Under certain regularity conditions, the maximizer of \eqref{eq-mle-fin-exp-fam}, denoted by $\hat{\theta}_{\ML}$, exists and is unique, and must satisfy the equation 
	\begin{align*} %\label{eq-moment-matching-fin-exp-fam}
		\nabla B \parens{\hat{\theta}_{\ML}} = \frac{1}{n} \sum_{i=1}^n \varphi \parens{X_i}, 
	\end{align*}
	which has the moment matching interpretation that the sample mean of the canonical statistic must match the population mean at the MLE, since $\nabla B \parens{\hat{\theta}_{\ML}} = \int_{\calX} \tilde{q}_{\hat{\theta}} \parens{x} \varphi \parens{x} \diff x$. In particular, the inverse map of $\nabla B$ exists and 
\begin{align*}
	\hat{\theta}_{\ML} = \parens{\nabla B}^{-1} \parens[\bigg]{\frac{1}{n} \sum_{i=1}^n \varphi \parens{X_i}}. 
\end{align*}
...
The first idea is to regularize the function space over which we minimize $\widehat{J}_{\NLL}$. Rather than minimize it over the entire $\calH$, \textcite{Fukumizu2005-mf} proposed to construct a sequence of nested finite-dimensional subspaces of $\calH$ that enlarges with the sample size $n$, $\sets{\hil^{\parens{m_n}}}_{m_n \in \Natural}$ satisfying $\calH^{\parens{m_n}} \subset \hil^{\parens{m_{n+1}}}$ for all $n$, and minimize $\widehat{J}_{\NLL}$ over $\hil^{\parens{m_n}}$. Supposing $\hat{f}_{\ML}^{\parens{m_n}} := \argmin_{f \in \calH^{\parens{m_n}}} \widehat{J}_{\NLL} \parens{f}$ exists for all $n$, \textcite{Fukumizu2005-mf} showed that, if $p_0 \in \calQ_{\ker}$, together with some additional assumptions, $q_{\hat{f}_{\ML}^{\parens{m_n}}}$ is consistent for $p_0$ under the KL-divergence. 

Even though this approach is theoretically interesting, it suffers from several theoretical and practical drawbacks. On the theoretical side, the consistency of $q_{\hat{f}_{\ML}^{\parens{m_n}}}$ relies on the decay rate of the smallest eigenvalue of certain covariance operator, which is hard or even impossible to check in practice. On the practical side, \textcites{Fukumizu2005-mf} did not elucidate guidelines on which class of RKHS should be used or how to choose the sequence of nested finite-dimensional subspaces. Moreover, even if such guidelines were provided, the minimization problem is nonlinear by its nature and one has to rely on an iterative optimization algorithm to compute $\hat{f}_{\ML}^{\parens{m_n}}$. Then, it is inevitable to deal with $A$ and its derivative, both of which involve integration over a possibly high-dimensional space and are hard to handle in practice. Thus, the density estimator constructed using this approach is not attractive. 
...
Since $\widehat{J}_{\NLL}$ depends on $A$, the classic representer theorem fails to characterize such a minimizer (see Chapter \ref{ch4} for more details). In addition, since $\calH$ is infinite dimensional, this unique minimizer in $\calH$ is computationally intractable. Instead, they proposed to minimize the penalized NLL loss functional over $\calH_{0} \oplus \widetilde{\calH}_n$, where $\calH_0 := \sets[\big]{f \in \calH \mid \widetilde{P} \parens{f} = 0}$ is the null space of $\widetilde{P}$ and $\widetilde{\calH}_n := \sets{f \mid f := \sum_{i=1}^n \alpha_i k \parens{X_i, \,\cdot\,}, \alpha_1, \cdots, \alpha_n \in \Real}$ is an $n$-dimensional subspace of $\calH$, and established asymptotic properties of $q_{\tilde{f}_{\ML}^{\parens{\lambda}}}$, where $\tilde{f}_{\ML}^{\parens{\lambda}} := \argmin_{f \in \calH_{0} \oplus \widetilde{\calH}_n} \sets[\big]{\widehat{J}_{\NLL} \parens{f} + \lambda \widetilde{P} \parens{f}}$. 
...
\textcite{Gu1993-lf} proposed an iterative algorithm to compute $\tilde{f}_{\ML}^{\parens{\lambda}}$ and used the quadrature rule over a dense mesh to approximate $A$ and its derivative at each iteration. All numerical examples therein were limited to cases $d \le 2$. If $d$ is large, the computation would become prohibitively expensive and the approximations via this approach could be very poor. 
...
\begin{align*}
	\maximize_{p \in \calP \cap L^2 \parens{\calX}} \ \minimize_{f, g \in \calH} & \, \bigg\{- \frac{1}{n} \sum_{i=1}^n f \parens{X_i} + \int_{\calX} \parens{f \parens{x} - g \parens{x}} p \parens{x} \diff x \nonumber \\ 
	& \qquad \quad + \int_{\calX} e^{g \parens{x}} \mu \parens{x} \diff x + \lambda \widetilde{P} \parens{f} \bigg\}, % \label{eq-dai-formualtion}
\end{align*}
and 
...
\end{align}
If we let $q = q_f \in \calQ_{\ker}$ in \eqref{eq-sm-loss}, under certain additional regularity conditions that will be made explicit in Chapter \ref{ch3}, we can rewrite $\widehat{L}_{\SM}$, up to additive constants, as 
\begin{align}
...
\end{align}
In \eqref{eq-hatC} and \eqref{eq-hatz}, with a fixed $x \in \calX$, $\partial_u^s k \parens{x, y} := \frac{\partial^s}{\partial w_u^s} k \parens{x, y}\big\vert_{w = x}$, for all $y \in \calX$, $s = 1, 2$, and $u = 1, \cdots, d$. 
%For $a, b \in \calH$, $a \otimes b$ defines a map from $\calH$ to $\calH$ via $\parens{a \otimes b} c = \innerp{b}{c}_{\calH} a$ for all $c \in \calH$. 
In $\parens{\star}$, we use $\partial_u^s k \parens{x, \,\cdot\,} \in \calH$ and the reproducing property of partial derivatives of $k$, $\partial_u^s f \parens{x} = \innerp{f}{\partial_u^s k \parens{x, \,\cdot\,}}_{\calH}$, for $s = 1, 2$ and $u = 1, \cdots, d$ \parencites{Zhou2008-jt}. Details about the partial derivatives of $k$ and their reproducing properties can be found in Section \ref{section-kernel-derivative} in Appendix \ref{app1}. In particular, notice that $\widehat{J}_{\SM}$ does \textit{not} involve $A$. 

Now, $\widehat{J}_{\SM}$ is a quadratic functional in $f$. It is not hard to show that the operator $\widehat{C}$ is linear, self-adjoint, and positive semidefinite. Hence, minimizing $\widehat{J}_{\SM}$ over $\calH$ is a convex optimization problem. However, $\hat{f}_{\SM} := \argmin_{f \in \calH} \widehat{J}_{\SM} \parens{f}$ does \emph{not} exist in general. Suppose that $\hat{f}_{\SM}$ exists. Then, $\hat{f}_{\SM}$ must satisfy the first-order optimality condition $\widehat{C} \hat{f}_{\SM} = \hat{z}$. Note that the range of $\widehat{C}$ is 
\begin{align*}
	\range \parens{\widehat{C}} := \mathrm{Span} \braces[\Big]{\partial_u k \parens{X_i, \,\cdot\,} \text{ for all } i = 1, \cdots, n \text{ and } u = 1, \cdots, d}, 
\end{align*}
and that $\hat{z}$ involves $\partial_u^2 k \parens{X_i, \,\cdot\,}$ for all $i = 1, \cdots, n$ and $u = 1, \cdots, d$. In general, $\hat{z} \notin \range \parens{\widehat{C}}$, except for some special choices of $k$, and $\hat{f}_{\SM}$ does not exist. 
%in other words, minimizing $\widehat{J}_{\SM}$ amounts to solving an infinite-dimensional linear system, and $\hat{f}_{\SM} = \widehat{C}^{-1} \hat{z}$. 

% However, since $\widehat{C}$ has finite rank and must be a compact operator in an infinite-dimensional RKHS $\calH$, it is \emph{not} invertible \parencite[Section 16.5 in][]{Royden2018-cd}. Therefore, $\hat{f}_{\SM}$ does \textit{not} exist. 
...
\end{align*}
the $\parens{\parens{i-1}d+u, \parens{j-1}+v}$-entry of $\bG \in \Real^{nd \times nd}$ is $\innerp{\partial_u k \parens{X_i, \,\cdot\,}}{\partial_v k \parens{X_j, \,\cdot\,}}_{\calH} = \partial_i \partial_{i+d} k \parens{X_i, X_j}$, and $\parens{\parens{i-1}d+u}$-entry of $\bh \in \Real^{nd}$ is 
\begin{align*}
...
Note that the matrix $\bG$ is positive semidefinite and $n \rho \bI_{nd}$ is positive definite, so their sum must be positive definite and must be invertible, and it follows that $\balpha^{\parens{\rho}} = \frac{1}{\rho} \parens{\bG + n \rho \bI_{nd}}^{-1} \bh$. 
...
\begin{proof}
	Let $f \in \calH$ be arbitrary. Due to the Cauchy-Schwartz inequality, we have $\abs[\big]{\innerp{f}{k \parens{x, \,\cdot\,}}_{\calH}} \le \norm{f}_{\calH} \norm{k \parens{x, \,\cdot\,}}_{\calH} \le \kappa_1 \norm{f}_{\calH}$. Then, we can bound $A \parens{f}$ from above by 
	\begin{align*}
		A \parens{f} \le & \, \log \parens[\bigg]{\exp \parens{\kappa_1 \norm{f}_{\calH} } \int_{\calX} \mu \parens{x}  \diff x} \stackrel{\parens{\star}}{=} \kappa_1 \norm{f}_{\calH} < \infty, 
	\end{align*}
	where we use the assumption that $\mu$ is a pdf over $\calX$ in deriving $\parens{\star}$. Since the choice of $f \in \calH$ is arbitrary, we conclude $\calH \subseteq \calF$. On the other hand, it is obvious that $\calF \subseteq \calH$. We conclude that $\calF = \calH$. 
\end{proof}
...
\begin{proof}[Proof of Lemma \ref{lemma-eval-functional-frechet-diff}]
	...
	\begin{align*}
		J_x \parens{f + g} - J_x \parens{f} = & \, \innerp{f + g}{k \parens{x, \,\cdot\,}}_{\calH} - \innerp{f}{k \parens{x, \,\cdot\,}}_{\calH} 
		= \innerp{g}{k \parens{x, \,\cdot\,}}_{\calH}. 
	\end{align*}
	Then, 
	\begin{align*}
		\frac{\abs[\big]{J_x \parens{f + g} - J_x \parens{f} - \innerp{g}{k \parens{x, \,\cdot\,}}_{\calH}}}{\norm{g}_{\calH}} = 0, 
	\end{align*}
	and, by Definition \ref{def-frechet-differentiable} in Appendix \ref{app1}, we conjecture $\Diff J_x \parens{f} \parens{g} = \innerp{g}{k \parens{x, \,\cdot\,}}_{\calH}$, for all $g \in \calH$. 
	
	We next verify that the map $\Diff J_x \parens{f}$ is linear and bounded. The linearity part follows from the linearity of inner product and is omitted. To establish the boundedness, we have, for all $g \in \calH$, 
	\begin{align*}
		\abs{\Diff J_x \parens{f} \parens{g}} = & \, \abs{\innerp{g}{k \parens{x, \,\cdot\,}}_{\calH}} 
		\le \norm{g}_{\calH} \norm{k \parens{x, \,\cdot\,}}_{\calH} 
		\le \kappa_1 \norm{g}_{\calH}, 
	\end{align*}
	where $\kappa_1 := \sup_{x \in \calX} \sqrt{k \parens{x, x}} < \infty$, by the assumption. Hence, $\Diff J_x \parens{f}$ is a bounded linear operator. We conclude that $J_x$ is Fr{\'e}chet differentiable at $f \in \calH$ with the Fr{\'e}chet derivative at $f \in \calH$ being $\Diff J_x \parens{f} \parens{g} = \innerp{g}{k \parens{x, \,\cdot\,}}_{\calH}$ for all $g \in \calH$. Since the choice of $f \in \calH$ is arbitrary, we conclude $J_x$ is Fr{\'e}chet differentiable over $\calH$. % and $\Diff J_x \parens{f} \parens{g} = \innerp{g}{k \parens{x, \,\cdot\,}}_{\calH}$ for all $g \in \calH$. 
	
	Using the definition of Fr{\'e}chet gradient, it is easy to see $\nabla J_x \parens{f} = k \parens{x, \,\cdot\,}$ for all $f \in \calH$. 
\end{proof}


\subsection{Proof of Proposition \ref{prop-frechet-derivative-A}}\label{subsection-proof-prop-frechet-derivative-A}
...
\begin{proof}[Proof of Proposition \ref{prop-frechet-derivative-A}]
	...
	Let $f \in \calH$ be arbitrary. We first show $J_1$ is Fr{\'e}chet differentiable at $f \in \calH$ with the Fr{\'e}chet derivative
	\begin{align*}
	...
	Note that the integrand of $\int_{\calX} \mu \parens{x} \exp \parens{ f \parens{x} } k \parens{x, \,\cdot\,} \diff x$ is an element in $\calH$, and 
	\begin{align*}
		...
		\le & \, \kappa_1 \exp \parens{A \parens{f}} < \infty, 
	\end{align*}
	where $\kappa_1 := \sup_{x \in \calX} \sqrt{k \parens{x, x}} < \infty$ by the assumption. Hence, $\int_{\calX} \mu \parens{x} \exp \parens{ f \parens{x} } k \parens{x, \,\cdot\,} \diff x$ is Bochner integrable with respect to the Lebesgue measure (by Proposition \ref{prop-bochner-integrability} in Appendix \ref{app1}), and we can interchange the inner product and the integral (by Proposition \ref{prop-properties-bochcher-integral}\ref{prop-properties-bochcher-integral-c} in Appendix \ref{app1}) and have 
	\begin{align*}
	...
	\begin{align*}
		...
		\stackrel{\text{(ii)}}{\le} & \, \int_{\calX} \mu \parens{x} \exp \parens{ \kappa_1 \norm{f}_{\calH} } \bracks[\Bigg]{ \sum_{m=2}^\infty \frac{\norm{g}_{\calH}^m \kappa_1^{m/2}}{m!}} \diff x \\ 
		\stackrel{\text{(iii)}}{=} & \, \exp \parens{ \kappa_1 \norm{f}_{\calH} } \bracks[\Bigg]{ \sum_{m=2}^\infty \frac{\norm{g}_{\calH}^m \kappa_1^{m/2}}{m!}}, 
	\end{align*}
	where (i) follows from the Cauchy-Schwartz inequality, (ii) is due to $\sqrt{k \parens{x, x}} \le \kappa_1$, and (iii) is because $\mu$ is a density function over $\calX$. To proceed, we have 
	\begin{align*}
		\frac{\abs[\big]{J_1 \parens{f + g} - J_1 \parens{f} - \Diff J_1 \parens{f} \parens{g}}}{\norm{g}_{\calH}} 
		\le \exp \parens{ \kappa_1 \norm{f}_{\calH} } \bracks[\Bigg]{ \sum_{m=2}^\infty \frac{\norm{g}_{\calH}^{m-1} \kappa_1^{m/2}}{m!}} 
		\to & \, 0, 
	\end{align*}
	as $\norm{g}_{\calH} \to 0$. 
	...
	\begin{align*}
		...
		% = & \, \norm{g}_{\calH} \int_{\calX} \mu \parens{x} \exp \parens{ \kappa_1 \norm{f}_{\calH} } \kappa_1 \diff x \nonumber \\ 
		\le & \, \bracks[\Big]{\kappa_1 \exp \parens{ \kappa_1 \norm{f}_{\calH} }} \norm{g}_{\calH}, 
	\end{align*}
	from which we conclude that $\Diff J_1 \parens{f}$ is a bounded operator. Thus, $J_1$ is Fr{\'e}chet differentiable at $f \in \calH$ with the desired Fr{\'e}chet derivative. 
	
	Since $A = J_2 \circ J_1$, applying Proposition \ref{prop-property-frechet-differentiable}\ref{prop-property-frechet-differentiable-b} in Appendix \ref{app1}, we obtain, for any $f \in \calH$, 
	\begin{align*}
...
%		\le & \, \sum_{v=2}^m {m \choose v} \norm{f}_{\calH}^{m-v} \norm{g}_{\calH}^{v-1} \kappa_1^{m/2} \to 0 
%	\end{align*}
...
%	\end{align}
%	In the derivation above, we use the Taylor expansion of the exponential function and the property that this Taylor expansion is absolutely convergent. Also, we use the Cauchy-Schwartz inequality and the assumption $\kappa_1 < \infty$ and apply Fubini's Theorem. 
%
...
\begin{proof}[Proof of Proposition \ref{prop-fin-ker-exp-fam-connection}]
	...
	\ref{prop-proof-connection-c} Finally, to show $\parens{\calH_0, \innerp{\,\cdot\,}{\,\cdot\,}_{\calH_0}}$ is a RKHS with the reproducing kernel \eqref{eq-m-dim-exp-fam-rk}, we let $x \in \calX$ be fixed and $E_x: \calH_0 \to \Real$ be the evaluation functional, i.e., $E_x \parens{f} = f \parens{x}$ for all $f \in \calH_0$, and need to show $E_x$ is a bounded operator. 
...
%	and 
	\begin{align*}
		...
		= M_x \norm{f}_{\calH_0}, 
	\end{align*}
	where $M_x := \sqrt{\sum_{j=1}^m \parens{\varphi_j \parens{x}}^2} < \infty$. Hence, the evaluation functional $E_x$ is a bounded operator, from which we conclude $\calH_0$ is a RKHS. 
...
%	
%	We are going to prove by contradiction and suppose $\hat{f}_{\ML} := \argmin_{f \in \calF} \widehat{J}_{\NLL} \parens{f}$ exists. 
%	
%	Since $\kappa_1 := \sup_{x \in \calX} \sqrt{k \parens{x, x}} < \infty$, Proposition \ref{prop-bounded-kernel-H=F} implies that $\calH = \calF$. Minimizing $\widehat{J}_{\NLL}$ over $\calH$ is an unconstrained optimization problem. Due to Fermat's rule \parencites[Theorem 16.3 in][]{Bauschke2017-he}, the minimizer $\hat{f}_{\ML}$ must satisfy $\nabla \widehat{J}_{\NLL} \parens{\hat{f}_{\ML}} = 0$, where $\nabla \widehat{J}_{\NLL}: \calH \to \calH$ is the Fr{\'e}chet gradient operator of $\widehat{J}_{\NLL}$. By the results from Lemma \ref{lemma-eval-functional-frechet-diff} and Proposition \ref{prop-frechet-derivative-A}, we have, for all $f \in \calH$, 
%	\begin{align*}
...
%	\end{align*}
%	Then, it is necessary that $\hat{f}_{\ML}$ satisfies 
%	\begin{align*}
%		\nabla A \parens{\hat{f}_{\ML}} = \frac{1}{n} \sum_{i=1}^n k \parens{X_i, \,\cdot\,}; 
%	\end{align*}
%	in other words, $\hat{f}_{\ML}$ is given by the inverse map of $\nabla A$. 
%	
%	However, this inverse map does \emph{not} exist, which can be seen by assessing the second-order Fr{\'e}chet derivative of $A$ given in Proposition \ref{prop-frechet-derivative-A-order2}. This is a self-adjoint trace-class operator, and by the Hilbert-Schmidt Theorem \parencites[Section 16.6 in][]{Royden2018-cd}, it has infinitely many eigenvalues clustered at 0, which is not invertible. It follows that the inverse map of $\nabla A$ does not exist and $\hat{f}_{\NLL}$ does not exist. 
%\end{proof}
