...
\begin{frame}{Introduction to density estimation problem}
	...
	\begin{itemize}
		\item Density estimation: Let $X_1, \cdots, X_n$ be i.i.d samples from an unknown pdf $p_0: \calX \to [0, \infty)$, where $\calX \subseteq \Real^d$. Estimate $p_0$ using these samples. 
		...
		\item Two categories of approaches: 
		\begin{itemize}
			\item parametric approach
			\item nonparametric approach (our focus)
		\end{itemize}
	\end{itemize}
...
\begin{frame}{Nonparametric approach to density estimation}
	
%	Makes fewer assumptions about $p_0$. 
%	
%	\vspace{10pt}
	
	We focus on methods that involve minimizing 
	\begin{align*}
		\widehat{L} \parens{q} + \lambda P \parens{q}, \qquad \quad \text{ subject to } q \in \mathcal{Q}, 
	\end{align*} 
	where 
	\begin{itemize}
		\item $\mathcal{Q}$ is a pre-specified class of pdfs on $\mathcal{X}$, 
		\item $\widehat{L}: \mathcal{Q} \to \mathbb{R}$ is a loss functional, 
		\item $P: \mathcal{Q} \to [0, \infty)$ is a penalty functional, and 
		\item $\lambda \ge 0$ is a penalty parameter. 
	\end{itemize}
	
\end{frame}


%\begin{frame}{Nonparametric approach to density estimation --- examples}
%	
%\end{frame}
...
\section{2. Density Estimation in Kernel Exponential Families}
...
\begin{frame}{Review of finite-dimensional exponential family}
	...
	% denoted by 
	$\mathcal{Q}_{\mathrm{fin}}$, contains all pdfs % of the form 
	\begin{align}
...
\begin{frame}{Assumptions on $\mu$, $\mathcal{H}$, and $k$}
	...
	\begin{itemize}
		...
		\item $k$ is 
		\begin{itemize}
			\item continuously twice differentiable, which ensures all quantities shown later to be well-defined, and 
...
\begin{frame}{Using the NLL loss functional}
...
\begin{frame}{Minimize penalized NLL loss functional}
...
%	where 
	...
	\begin{itemize}
		...
		\end{align*}
		\textcite{Gu1993-lf} proposed an iterative algorithm to compute the minimizer. \\ Need to work with {\color{red}$A$ and its derivatives} (involving integration over a possibly high-dimensional space)  --- {\color{red} difficult task!}
...
\begin{frame}{Use the SM loss functional}
...
\section{3. Early Stopping Score Matching Density Estimator}
...
\begin{frame}{Numerical example}
	...
	\begin{columns}[c]
	    \begin{column}{.5\textwidth}
			...
			{\color{defaultcolor} Goal:} Illustrate the similarity between penalized and early stopping SM density estimators. 
	...
	\begin{columns}[c]
	    \begin{column}{.3\textwidth}
			...
			\begin{itemize}
				...
				\item The RKHS $\mathcal{H}$ is the one generated by 
				%the Gaussian kernel 
				\begin{align*}
					k \parens{x, y} = \exp \parens[\bigg]{-\frac{\parens{x - y}^2}{2 \sigma^2}}
				\end{align*}
				with $\sigma = 5$. 
				
%				\vspace{5pt}
%				
%				\item In computing early stopping density estimates, we use $\tau = 2$. 
			\end{itemize}
...
\section{4. Comparison of Regularized Density Estimators}
...
\begin{frame}{Penalized ML density estimator}
	...
	{\color{defaultcolor}Remedy:} Use a finite-dimensional space 
	\begin{align}
...
\begin{frame}{Computation of regularized SM density estimators}
	...
	\begin{itemize}
		...
		\end{align*}
		where $\parens{\tilde{\beta}_1^{\parens{t}}, \cdots, \tilde{\beta}_m^{\parens{t}}}^\top \in \mathbb{R}^m$ can be obtained by matrix calculation. % multiplication and addition of certain matrices of size at most $m \times m$. 
	\end{itemize}
...
\section{5. The Sensitivity of Density Estimators via the Influence Function}
...
\begin{frame}{Goal}
	...
	To develop a set of tools to understand the sensitivity of density estimators. 
...
\begin{frame}{Influence functions of log-density function evaluated at $x$}
	...
	Let $T$ be a map from the collection of distributions over $\calX$ to the class of log-density functions over $\calX$. 
%	\begin{itemize}
%		\item 
...
\begin{frame}{A special sample influence function}
	...
	\end{align*}
	where $F_{n+1}$ is the empirical distribution of $X_1, \cdots, X_n$ and $y$. 
...
%	
%	Let $G$ be a distribution over $\mathcal{X}$. 
%	
%	\vspace{10pt}
%	
%	Suppose $T (G) = \log q_{f_G}$, where $q_{f_G} \in \mathcal{Q}_{\mathrm{ker}}$ for some $f_G \in \mathcal{H}$. Then, 
%	\begin{align*}
...
\begin{frame}{Notation}
	...
	To facilitate the comparison, %the sensitivity of penalized ML and SM density estimators in $\mathcal{Q}_{\mathrm{ker}}$, 
	define 
	\begin{align*}
...
%\begin{frame}{Plotting $\rho$ on one axis is a {\color{red} bad} idea}
%
%	\begin{columns}[c]
...
%	    	\vspace{10pt}
%	    	
%	    	Recall our goal is to compare sensitivity of penalized ML and SM density estimators. 
%	    	
%	    	\vspace{10pt}
%	    	
...
\begin{frame}{Summary}
	...
	\begin{itemize}
		...
		\item are very sensitive to the presence of an isolated observation, especially when penalty parameter is small. 
	\end{itemize}
